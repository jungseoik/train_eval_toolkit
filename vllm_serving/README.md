...
async def main():
    async with VLLMClient(
        ...
        model="Qwen/Qwen3.5-2B",  # .env의 MODEL_NAME과 동일해야 함
...
| `model` | `Qwen/Qwen3.5-2B` | 모델명 (**서버의 MODEL_NAME과 일치 필수**) |
...
MODEL = "Qwen/Qwen3.5-2B"         # 서버의 MODEL_NAME과 동일
...
| 변수명 | 기본값 | 설명 |
|---|---|---|
| `MODEL_NAME` | `Qwen/Qwen3.5-2B` | HuggingFace 모델 ID |
