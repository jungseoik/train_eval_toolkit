...
5. [API 구조 (HTTP 통신)](#api-구조-http-통신)
6. [Python 클라이언트 사용법](#python-클라이언트-사용법)
7. [응답 파싱](#응답-파싱)
8. [설정 레퍼런스](#설정-레퍼런스)
9. [트러블슈팅](#트러블슈팅)
...
KV_CACHE_MEMORY_BYTES=20G # H100 80G 기준, 여유 있게 설정
```
...
- **미설정 (기본)**: vLLM이 `GPU_MEMORY_UTILIZATION` 기반으로 자동 계산
- **명시적 설정**: 모델 로딩 후 남는 메모리에서 KV Cache에 사용할 양을 직접 지정

GPU VRAM이 넉넉하면 미설정(자동)이 편하고, 여러 프로세스가 GPU를 공유하는 환경이면 명시적으로 제한하는 것이 안전하다.
...
`POST /v1/chat/completions` 에 JSON body를 전송한다:
...
| `KV_CACHE_MEMORY_BYTES` | (자동) | KV Cache 메모리 상한 (예: `10G`, `20G`). 미설정 시 자동 계산 |
...
1. `KV_CACHE_MEMORY_BYTES`를 명시적으로 제한 (예: `5G`)
