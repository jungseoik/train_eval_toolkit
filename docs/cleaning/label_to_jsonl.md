...
프롬프트는 `configs/label_convert/prompts.yaml`에서 계층 구조로 관리됩니다.
...
> **새 작업 추가**: `configs/label_convert/prompts.yaml`에 task_name 블록을 추가하면 코드 변경 없이 바로 사용할 수 있습니다.
>
> **미정의 task_name**: `train` 모드에서 YAML에 없는 task_name을 지정하면 사용 가능한 목록과 함께 오류가 출력됩니다. `test` 모드에서는 프롬프트가 불필요하므로 미정의 task_name도 정상 동작합니다.
...
| `configs/label_convert/prompts.yaml` | 프롬프트 템플릿 정의 (task_name × data_type) |
