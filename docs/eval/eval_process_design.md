...
두 파이프라인은 동일한 subprocess 분리 + Docker 재시작 아키텍처를 공유하며,
`pipeline.mode: "vllm" | "lmdeploy"` 필드로 API/CLI 레벨에서 디스패치된다.
...
사용자
  ...
  └─ API 요청: curl -F 'file=@config.yaml' http://server:9000/pipeline/run/file
     └→ main.py (FastAPI/uvicorn)
        ...
        │  공통 파이프라인 흐름                           │
        │                                                 │
        │  1. YAML 검증 + pipeline.mode 확인 + 설정 로드  │
        │  2. MODEL 단계                                  │
        │     - LMDeploy: 로컬 model_path 검사 + HF 다운로드│
        │     - vLLM:    HF 캐시 확인 + Unsloth 토크나이저 │
        │                자동 패치 (tokenizer_patcher)    │
        │  3. Docker 컨테이너 기동 (vLLM/LMDeploy 서버)   │
        │  4. 서버 준비 대기 (health check /v1/models)    │
        │                                                 │
        │  5. 벤치마크 평가 루프                          │
        │     ┌─────────────────────────────────────┐     │
        │     │ 벤치마크 1                          │     │
