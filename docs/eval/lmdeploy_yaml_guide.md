...
아래 템플릿을 복사한 뒤, `# <-- 변경` 표시된 필드만 수정하면 새 모델 평가를 바로 실행할 수 있습니다.
`# <-- 고정값` 표시된 필드는 서버 환경에 종속된 값이므로 변경하지 마세요.

> 이 템플릿은 `PIA_AI2team_VQA_falldown.yaml`(Falldown 평가) 기준입니다. Fire 평가 시 변경 사항은 [Fire 평가 시 변경 사항](#fire-평가-시-변경-사항) 섹션을 참조하세요.
