...
def overlay_text_top_center(frame_bgr: np.ndarray, text: str) -> np.ndarray:
    ...
    # 텍스트
    color = (255, 255, 255) if text == "normal" else (0, 180, 255)  # normal: white, falldown: orange-ish
...
def process_video(
    ...
    # 윈도우 마지막 인덱스 리스트
    # window_last_indices = list(range(min(window_size-1, total_frames-1), total_frames, window_size))
    window_rep_indices = list(range(0, total_frames, window_size))
    # 배치 추론용 대표 프레임 RGB 이미지 모으기
    categories_per_window = []

    # 대표 프레임 추출을 위해 한 번 프레임 전체를 순회하며 캐싱(메모리 여유가 없으면 직접 랜덤 액세스 로직으로 대체)
    # 여기서는 메모리 절약을 위해 대표 프레임만 개별 접근:
    def read_frame_rgb(idx: int) -> np.ndarray:
        cap.set(cv2.CAP_PROP_POS_FRAMES, idx)
        ok, frame_bgr = cap.read()
        if not ok or frame_bgr is None:
            # 실패 시 이전/다음 프레임으로 폴백 시도
            return None
