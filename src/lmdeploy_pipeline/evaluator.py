...
def _benchmark_worker(
    ...
    # evaluate_benchmark이 _update_video_progress를 호출할 때
    # progress_file에 기록하도록 설정
    progress_state = {
...
def _poll_progress_file(
    ...
    자식 프로세스가 progress_file에 기록한 video/frame 정보를
    부모의 progress_state dict에 반영한다.
    """
        ...
        try:
            if os.path.exists(progress_file):
                with open(progress_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
                # 자식의 bench_idx=0 데이터를 부모의 실제 bench_idx에 반영
...
def run_evaluation(
    ...
    for idx, bench_name in enumerate(benchmarks):
        ...
        try:
            ...
            proc.start()
            proc.join()  # 완료 대기 (내부에서 concurrency 병렬 처리 유지)
            # → 프로세스 종료 → OS가 메모리 전부 회수
