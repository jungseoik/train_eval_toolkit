...
set -x
## h 100 누가 사용중
# GPUS=4 PER_DEVICE_BATCH_SIZE=1 
GPUS=4 PER_DEVICE_BATCH_SIZE=3
## h 100 누가 사용중
...
MERGE_DIR="InternVL3-2B_hyundai_8_20_add_${EPOCHS}"
...
PYTHONPATH="$(pwd)" python src/evaluation/evaluate_image_classfication.py \
    --checkpoint "${CHECKPOINT}" \
    --annotation data/instruction/evaluation/test_hyundai_hard_negative_2st_box_image_falldown.jsonl \
    --image-root data \
    --out-dir "${OUT_DIR}" \
    --batch-size "${BATCH_SIZE}" \
    --multi-gpu
...
PYTHONPATH="$(pwd)" python src/evaluation/evaluate_image_classfication.py \
    --checkpoint "${CHECKPOINT}" \
    --annotation data/instruction/evaluation/test_hyundai_01_16_QA_image_falldown.jsonl \
    --image-root data \
    --out-dir "${OUT_DIR}" \
    --batch-size "${BATCH_SIZE}" \
    --multi-gpu
...
PYTHONPATH="$(pwd)" python src/evaluation/evaluate_image_classfication.py \
    --checkpoint "${CHECKPOINT}" \
    --annotation data/instruction/evaluation/test_hyundai_01_27_QA_image_falldown.jsonl \
    --image-root data \
    --out-dir "${OUT_DIR}" \
    --batch-size "${BATCH_SIZE}" \
    --multi-gpu
